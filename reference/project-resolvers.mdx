...
Codanna is a local code intelligence MCP server and CLI for AI coding agents. Project resolvers read build configuration files (`go.mod`, `pyproject.toml`, `tsconfig.json`, `composer.json`, `pom.xml`, `build.gradle.kts`, `.csproj`, `Package.swift`) to derive module paths and source roots for cross-file relationship tracking and import resolution.
