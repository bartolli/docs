...
Codanna is a local code intelligence MCP server and CLI for AI coding agents. This guide configures TLS/SSL encryption with self-signed certificates for the HTTPS MCP server. Examples use Claude Code, but the method applies to other Node.js-based MCP clients.
...
<CardGroup cols={2}>
  <Card title="MCP Network" icon="globe" href="/reference/mcp-network">
    HTTP and HTTPS server modes for multiple clients.
  </Card>
  <Card title="MCP Persistent" icon="plug" href="/reference/mcp-persistent">
    Stdio transport — alternative to HTTPS for single-client setups.
  </Card>
</CardGroup>
