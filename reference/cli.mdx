...
<Note>
An index built by a binary with different output semantics is refused before any tool runs: read commands exit `7` with a message naming both versions, and `codanna index` rebuilds from scratch automatically. This prevents mixed-semantics results after an upgrade.
</Note>
