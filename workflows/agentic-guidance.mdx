...
Codanna is a local code intelligence MCP server and CLI for AI coding agents. Each tool result includes a follow-up hint that suggests the next logical action, guiding AI assistants through systematic codebase exploration.
...
**Text output:**
```
💡 Found 3 matches. Consider using 'find_symbol' on the most relevant result...
```
...
<Steps>
  <Step title="Search for the concept">
    ...
    💡 Found 3 matches. Consider using 'find_symbol' on the most relevant
  ...
  <Step title="Get symbol details">
    ...
    💡 Symbol found with full context. Explore 'get_calls' to see what it calls,
  ...
  <Step title="Check who uses it">
    ...
    💡 Found 30 callers. Consider 'analyze_impact' for complete dependency graph
  ...
  <Step title="Understand full impact">
    ...
    💡 Significant impact with 76 affected symbols. Consider breaking
